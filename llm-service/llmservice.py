...
class LlmService:
    ...
    def __init__(self, model_name, run_cmd: str, model_path: str, tokenizer_py: str, tokenizer_path: str, tokenizer_port: int, include_thinking: str):
    ...
    def prompt_llm(self, prompt: str) -> str:
        ...
        self.__change_status(LlmStatus.THINKING)
    ...
    def start_model(self):
        ...
        # Now start Model
        logger.info(f"Starting model {self.model_path}/{self.run_cmd} listening at port {self.tokenizer_port}")
    ...
    def __process_llm_output(self, line):
        ...
        # ERROR states
        if INIT_FAILED_PATTERN.match(line) != None or SEG_FAULT_PATTERN.match(line) != None:
        ...
        # PROMPT_ANSWERED state
        elif HIT_EOS_PATTERN.match(line) != None:
            self.__change_status(LlmStatus.PROMPT_ANSWERED)
            self.hit_eos_msg = line
            logger.info("Model has answered!")
        elif self.get_status() == LlmStatus.THINKING:
            self.llm_response_buffer.write(line)
        else:
            logger.warn(f"Message {line} from llm is ignored!")
    ...
    def __start_tokenizer_and_await_readiness(self):
        ...
        # Now wait for tokenizer readiness
    ...
    def __change_status(self, new_status: LlmStatus):
        ...
        if new_status == LlmStatus.PROMPT_ANSWERED:
